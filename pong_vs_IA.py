...
ball.dy = (120 * random.choice(dcs) / fps)*1.85
...
def writeScore():
    score_t.clear()
    score_t.write("Joueur 1: " + str(scoreA) + "   Ordinateur: " + str(scoreB), align="center",
...
while True:
    ...
    if ball.ycor() > paddle_b.ycor():
        if iaCount <= iaMaxCount:
            iaCount += 1
        else:
            pb_move_up()
            iaCount = 0

    elif ball.ycor() < paddle_b.ycor() + 100:
        if iaCount <= iaMaxCount:
            iaCount += 1
        else:
            pb_move_down()
